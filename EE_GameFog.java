...
/**
 * Simulation setup for case study 1 - EEG Game

 */
public class EE_GameFog {
	...
	public static void main(String[] args) {
		...
		Log.printLine("Starting EE_GameFog...");
		...
		try {
			...
			String appId = "EE_GameFog"; // identifier of the application
			...
			Log.printLine("EE_GameFog finished!");
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static void createFogDevices(int userId, String appId) {
		...
		cloud.setParentId(-1);
		FogDevice proxy = createFogDevice("proxy_server", 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333); // creates the fog device Proxy Server (level=1)
	...
	private static FogDevice addGw(String id, int userId, String appId, int parentId){
		FogDevice dept = createFogDevice("WiFiRooter_"+id, 2800, 4000, 10000, 10000, 1, 0.0, 107.339, 83.4333);
		fogDevices.add(dept);
	...
	private static FogDevice addMobile(String id, int userId, String appId, int parentId){
		FogDevice mobile = createFogDevice("mobile_"+id, 1000, 1000, 10000, 270, 3, 0, 87.53, 82.44);
		mobile.setParentId(parentId);
		
		Sensor eegSensor = new Sensor("eeg_"+id, "EEG", userId, appId, new DeterministicDistribution(EEG_TRANSMISSION_TIME)); // inter-transmission time of EEG sensor follows a deterministic distribution
		sensors.add(eegSensor);
		
		Actuator display = new Actuator("lcd_"+id, userId, appId, "DISPLAY");
	...
	private static Application createApplication(String appId, int userId){
		...
		application.addAppEdge("client", "concentration_calculator", 3500, 500, "SENSOR", Tuple.UP, AppEdge.MODULE); // adding edge from Client to Concentration Calculator module carrying tuples of type _SENSOR
		 ...
		 */
		application.addTupleMapping("client", "EEG", "SENSOR", new FractionalSelectivity(0.9)); // 0.9 tuples of type _SENSOR are emitted by Client module per incoming tuple of type EEG 
		...
		application.addTupleMapping("concentration_calculator", "SENSOR", "CONCENTRATION", new FractionalSelectivity(1.0)); // 1.0 tuples of type CONCENTRATION are emitted by Concentration Calculator module per incoming tuple of type _SENSOR 
